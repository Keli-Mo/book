...
- 最低微信基础库为 2.3.0；录音固定推荐 `16000Hz`、单声道、`48000bps` MP3，五分钟约 1.8MB。实际时长取 `onStop.duration`；文件大小和 SHA-1 以最终保存文件的实测结果为准，不能要求回调大小与最终文件完全相等。
- 系统中断结束后仅提示用户手动恢复，绝不自动调用 `resume()` 或自动偷录。
- 失败后尽量保留用户录音，禁止在结果不确定时删除云文件。
- 待上传录音最多 3 条、合计预算 8MiB，实际保存还受微信文件接口配额约束。未提交录音不再按 7 天自动删除；只清理明确不存在的文件引用，访问错误时保留引用。超限提示用户处理指定旧录音或联网提交；已保存文件修正大小后超限仍保留恢复信息，后续按实际累计量限制保存。
...
export type PendingCheckIn = {
  ...
  contentSha1?: string; // 旧记录可缺省，首次提交前按实际文件补齐。
  cloudFileId: string;
...
- [ ] 写 requestId、原生时长、保存文件实际大小和 SHA-1、`savedFilePath` 替换临时路径、最多 3 条/合计 8MiB，以及 8 天/365 天未提交录音仍保留和清理失败测试。
- [ ] 写保存失败时仍保留当前临时路径但标记不可恢复的测试。
- [ ] 写超过队列条数或 8MiB 时只清理明确丢失文件的引用、仍超限则不删除有效项且提示处理的测试。
- [ ] 运行测试确认模块缺失失败。
- [ ] 用注入的 storage/file adapter 实现，按实际文件大小记账；旧无指纹项首次提交时校准并持久化，云端仍核对实际大小和摘要。文件访问异常不得当作不存在。用户明确删除或云端确认提交后才删除仍存在的本地录音。
- [ ] 运行测试确认通过。
- [ ] 提交 `feat: 持久保存待上传跟读录音`。
