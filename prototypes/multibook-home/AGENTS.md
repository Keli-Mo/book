...
- 采用用户确认的“方案 1”：暖白纸张质感、深绿色主色、橙色重点动作，首页以“继续学习”作为首要入口。
- 首页只展示系列概览与当前学习进度；完整教材清单进入“全部教材”页查看，避免多本教材全部堆在首页。
- 教材页与示范音频必须按稳定的教材、训练、资源页编号关联；图片保持原比例，播放热点使用相对位置随图片缩放。
- 训练目录只在跟读页内出现；录音控制固定在底部，并支持录音、暂停、继续和完成打卡。
- 当前仅 CASA 第 1 册标记为可跟读，其余真实书封统一显示“待上线”，不伪造资源完成状态。
